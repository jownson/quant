...
def solve_bcl_data(csv_file):
    ...
    for data in bcl['数据']:
        ...
        #    print(str(float(need_volt)/10) +', ' + str(float(need_curr)/10))
        i = i + 1;

    return res;


df = pd.read_csv('E:\\workspace\\python\\quant\\demo\\BMS.csv', encoding ='gb2312')

res = solve_bcl_data(df)

res.to_csv('E:\\workspace\\python\\quant\\demo\\res.csv', encoding ='gb2312');
